...
async function searchCity() {
  try {
    ...
    if (temp.length > 3) {
      fontSize = "3.5";
      heroTemp.innerHTML = `<p style="font-size: ${fontSize}rem;">${JSON.parse(
        temp
      ).toFixed(0)}\u00B0</p>`;
  ...
  try {
    ...
    if (favourites !== null) {
      ...
      const mappedFavourites = favouriteArrayData.map((data) => {
        const favUlList = document.getElementsByClassName(
          "fav-dropdown-menu-1"
        );
              ...
              <div class="fav-dropdown-menu" id="${
                favLocation + "fav-dropdown-menu"
                  ...
                  <div class="fav-weather-box">
                      <div class="fav-item-4" id="${
                        favLocation + "fav-item-4"
                      }">
                          <p>${(() => {
                            let out;
                            const formattedDate = new Date(date);
                            const month = formattedDate.toLocaleString(
                              "default",
                              { month: "short" }
                            );
                      ...
                      <div class="fav-item-5" id="${
                        favLocation + "fav-item-5"
                      }">
                          <p>${"H: " + tempHigh.toFixed(0)}°</p>
                          <p>${"L: " + tempLow.toFixed(0)}°</p>
                      </div>
                      <div class="fav-item-6" id="${
                        favLocation + "fav-item-1"
                      }">
                              ...
                              for (
                                ...
                                out += `
                                      <tr>
                                          ...
                                          <td>${hourlyData[i].temp_c.toFixed(
                                            0
                                          )}&deg;</td>
  ...
  try {
    ...
    switch (condition) {
      ...
      case "Måttligt eller tungt snöblandat regn":
        ...
        if (x.matches) {
          hero.style.backgroundImage =
            "url(/img/mobile/conditions/drizzle.jpg)";
        } else {
          hero.style.backgroundImage =
            "url(/img/desktop/conditions/drizzle.jpg)";
        }
        break;
      //Åska
      ...
      case "Måttligt eller tungt snöfall i område med åska":
        ...
        if (x.matches) {
          hero.style.backgroundImage =
            "url(/img/mobile/conditions/thunder.jpg)";
        } else {
          hero.style.backgroundImage =
            "url(/img/desktop/conditions/thunder.jpg)";
        }
        break;
      //Moln
      case "Molnigt":
      ...
      case "Iskorn":
        ...
        if (x.matches) {
          hero.style.backgroundImage =
            "url(/img/mobile/conditions/snowing.jpg)";
        } else {
          hero.style.backgroundImage =
            "url(/img/desktop/conditions/snowing.jpg)";
        }
        break;
      //Soligt
      case "Soligt":
...
/////////////////////////////////////////////////////////////////////////////
const apiKey = "fbaac3dda9f24220ae681758242005";
...
const weatherInfoAndDisplay = async (apiKey, lastDisplayed) => {
  ...
  try {
    ...
    const rainChance = data1.forecast.forecastday[0].day.daily_chance_of_rain;
...
lista.addEventListener("click", () => {
  /* När hamburgermenyn klickas på så.. */
